...
class KMeansBase(object):
    ...
    def __init__(self, n_clusters = 8, init = "random", distanceType="jaccard",max_iter = 300, random_state = None, n_init = 10, tol = 1e-4):
        self.k = n_clusters # 聚类个数
        self.init = init # 输出化方式
        self.distanceType=distanceType #距离度量方法
    ...
    def assign_points(self, dataset, centers):
        ...
        for point in dataset:
            ...
            for i in range(len(centers)):
                val=distance(point[np.newaxis][0], centers[i],self.distanceType)
...
if __name__ == "__main__":
    ...
    km = KMeansBase(5,distanceType="euclidean")
    startTime = time.time()
